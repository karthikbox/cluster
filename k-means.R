...
data1 <- read.table(file='/home/karthik/r/jdt.csv', sep=',', header=T)
...
kclust.out.p <- prop.table(as.matrix(kclust.out),1)*100
out <- capture.output(describe.by(kclust.out,kclust))
cat(out,file='Kmeans_out.txt', sep='\n', append=F)

# clusterSum=function(x){
#   clust=subset(kclust.out,kclust==x)
#   sum= sum(clust$bugs)
#   return sum
# }

# for(i in 1:clust.level){
#   clust=subset(kclust.out,kclust==i)
#   sum= sum(clust$bugs)
#   print(sum)
# }
